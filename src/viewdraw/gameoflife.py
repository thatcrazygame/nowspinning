...
class GameOfLife(ViewDrawer):
    ...
    def new_random_grid(self, cutoff=INIT_CUTOFF):
        ...
        random = rng.integers(low=0, high=RNG_RANGE,
    ...
    def get_display_grid(self):
        ...
        arr = np.array([ALIVE_RGB if cell == ALIVE else DEAD_RGB 
                        for cell in data])
    ...
    def tick(self):
        ...
        new_grid = (neighbors == 3) | ((self.grid_data == ALIVE) 
                                       & (neighbors == 2))
        new_grid = np.asarray(new_grid.astype(int))
    ...
    def draw_gens_counter(self, canvas):
        ...
        canvas.SetImage(img)
        DrawText(canvas, font, padding, font.height, Color(45, 90, 224),
                 gens_str)
